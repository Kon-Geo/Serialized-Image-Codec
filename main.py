...
class SIC():
    ...
    def encode(self, input_file, output_file):
        ...
        img = imread(input_file)
        rows, cols, _ = img.shape
        # Create a 'buffer' variable to store the pixels of the image
        buffer = ""
        # Iterate through the rows and columns of the image
        for i in range(rows):
            for j in range(cols):
                ...
                pixel = img[i, j]
                hex_value = ('%02x%02x%02x' % (pixel[0], pixel[1], pixel[2])).upper()
                # Append the pixel to the buffer
                buffer += hex_value + ","
            buffer = buffer[:-1] + "\n" # Remove trailing comma
        buffer = buffer[:-1] # Remove trailing newline character
        # Create the new image file and write the buffer to it
        with open(output_file, "w+") as wf:
            wf.write(buffer)

    def decode(self, input_file, output_file):
        # Open input file and read the data in lines
        with open(input_file, "r") as rf:
            data = rf.readlines()
        # Create a blank numpy array serving as the new image
        new_image = zeros((len(data), len(data[0].split(",")), 3), uint8)
        # Iterate through the rows and columns of the input image and change them accordingly in the new image
        for i, line in enumerate(data):
            for j, y in enumerate(line.split(",")):
                new_image[i][j] = tuple(int(y[i:i+2], 16) for i in (0, 2, 4))
        # Save the new image
...
class Parser(ArgumentParser):
    def __init__(self):
        ...
        self.add_argument("-c", "--converter", help="The converter you want to use. (csic: Use compression)", choices=["csic", "sic"], default="csic")
        ...
        if args.random:
            ...
            print("usage: main.py [-h] -i INPUT [-e] [-d] [-r RANDOM] [-o OUTPUT] [-c {csic,sic}] [-v]\nmain.py: error: the following arguments are required: -i/--input")
            return
        # Check the converter the user wants to use and pass the argument to the corresponding class
        if args.converter == "csic":
            CSIC(args.input, args.encode or args.decode or "encode", args.output)
        if args.converter == "sic":
